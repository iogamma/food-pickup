...
// we are returning the id so might need nother spot in the callback function on the
  ...
  // retrieveData: (userId, done) => {
  //   db.select('id')
  //     .from('orders')
  //     .where({user_id:userId})
  //     .andWhere({status:null})
  //     .returning('id').then(function(ids) {
  //       db.select('quantity','name','price').from('items_orders').leftOuterJoin('items','items_orders.item_id','items.id').where({order_id:ids[0]});
  //     });
  // },
  ...
  updateCurrentOrder: (userId, newStatus, done) => {
    db.select('id').from('orders').where({id:userId}).andWhere({status:null}).update({status: newStatus, time_of_order:new Date()}).then(done)
  },
