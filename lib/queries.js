...
// we are returning the id so might need nother spot in the callback function on the
  ...
  //retrieve name, price and quantity data
  retrieveData: (done) => {
    db.select('id')
      .from('orders')
      .where({status:''})
      .returning('id').then(function(ids)) {
        db.select('image','name','price').from('items_orders').leftOuterJoin('items','item_id','items.id').where({order_id:ids[0]});
      });
  }
  ...
  updateCurrentOrder: (newStatus, done) => {
    db.select('id').from('orders').where({id:userId}).andWhere({status:''}).update({status: newStatus, time_of_order:new Date()}).then(done)
  },
    ...
    db.select('id')
      ...
      .then((rows) => {
        if (rows.length) {
          ...
          db.insert({order_id:currentOrder, item_id:itemId}).into('items_orders').then(done)
        }
      })
  },
  ...
  findCurrentOrder: (userId, done) => {
    db.select('id').from('orders').where({id:userId}).andWhere({status:''}).then(done);
  },
